...
public:
    ...
    // Helper function for in-order traversal and conversion
    void convertToDLL(Node* root) {
        ...
        if (prev == NULL) {
            // This condition is true for the leftmost node (first node of DLL)
            head = root;  // Set head as the leftmost node
        } else {
...
// Utility function to print the doubly linked list from left to right
...
// Utility function to print the doubly linked list from right to left
void printDLLReverse(Node* head) {
    if (head == NULL) return;
    Node* temp = head;

    // Move to the end of the DLL
